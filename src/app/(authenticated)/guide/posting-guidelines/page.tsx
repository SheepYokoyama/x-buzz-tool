  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      ...
      <Section icon={ShieldAlert} iconColor="#ef4444" title="絶対にやらないこと（両プラットフォーム共通）">
        <div className="rounded-xl overflow-hidden border border-slate-900/10">
        ...
        <Note color="rose">
          Xpresso はブラウザ自動化・非公式 SDK を一切使用していません。残るのは <span className="text-slate-800 font-medium">運用側のリスク</span> です。
        </Note>
      </Section>

      {/* ── X の現実 ──────────────────────────── */}
      <Section icon={TrendingDown} iconColor="#2563eb" title="X 側の現実（2026 年）">
        <ul className="space-y-2 text-[14px] text-slate-700">
          <Bullet>
            公式 API + OAuth + <span className="text-slate-900 font-medium">1 日 100 投稿以内</span> なら shadowban 対象外
          </Bullet>
          <Bullet>
            新規開発者は <span className="text-slate-900 font-medium">Pay-Per-Use 強制</span>：書込 $0.015/件、URL 含有 $0.20/件（2026-04 改定）
          </Bullet>
          <Bullet>
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody className="text-slate-700">
              <tr className="border-t border-slate-900/10">
                ...
                <td className="px-3 py-2 text-amber-600 font-mono">−30〜50%</td>
              </tr>
              <tr className="border-t border-slate-900/10 bg-slate-900/[0.02]">
                <td className="px-3 py-2">ハッシュタグの多用</td>
                <td className="px-3 py-2 text-amber-600 font-mono">−40%</td>
              </tr>
              <tr className="border-t border-slate-900/10">
                <td className="px-3 py-2">スパム通報 1 件</td>
                <td className="px-3 py-2 text-rose-600 font-mono">−369x（実質配信停止）</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {/* ── 推奨フロー ──────────────────────── */}
      <Section icon={MessageSquare} iconColor="#059669" title="推奨：「最初のコメント」にリンクを置く">
        <p className="text-[14px] text-slate-600 leading-relaxed">
          外部リンクは投稿本文に直接書かず、<span className="text-slate-800 font-medium">スレッドの 2 件目（リプライ）として送る</span> のがリーチ減衰を回避する定番テクニックです。
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          ...
          >
            <p className="text-[12px] font-semibold text-rose-600 mb-2 flex items-center gap-1.5">
          ...
          >
            ...
            <p className="text-[13px] text-slate-700 leading-relaxed">
              ...
              <span className="text-slate-500 text-[11px]">↓ リプライ（スレッド 2 件目）</span><br />
              「https://example.com/my-tool」
            </p>
          </div>
        </div>
        <Note>
          Xpresso の「ポスト作成」ページで <span className="text-slate-800 font-medium">スレッド（リプ連結）</span> モードを使えばこの形を簡単に作れます。
        </Note>
      </Section>

      {/* ── Threads の現実 ───────────────────── */}
      <Section icon={ShieldCheck} iconColor="#7c3aed" title="Threads 側の現実（緩い・無料）">
        <ul className="space-y-2 text-[14px] text-slate-700">
          <Bullet>
            API は <span className="text-slate-900 font-medium">完全無料</span>。1 プロファイル <span className="text-slate-900 font-medium">250 投稿/24h</span>・返信 <span className="text-slate-900 font-medium">1,000/24h</span>
          </Bullet>
          <Bullet>
            テキスト上限 <span className="text-slate-900 font-medium">500 文字</span>、メディアは公開アクセス可能な URL 必須
          </Bullet>
          <Bullet>
            「API だから降格」は <span className="text-slate-900 font-medium">公式・実測ともに根拠なし</span>。差はあっても API/ネイティブで 10% 程度
          </Bullet>
          <Bullet>
            支配的要因は <span className="text-slate-900 font-medium">投稿直後 30 分のエンゲージメント速度と返信の深さ</span>
          </Bullet>
        </ul>
        <Note>
      ...
      {/* ── EU AI Act ───────────────────────── */}
      <Section icon={FileWarning} iconColor="#7c3aed" title="EU AI Act Article 50（2026-08 施行）">
        <p className="text-[14px] text-slate-600 leading-relaxed">
          2026 年 8 月 2 日から、AI 生成テキスト・画像・音声・動画は以下 2 層の表示義務が課されます。違反責任は投稿主体に及びます。
        </p>
        <ul className="space-y-2 text-[14px] text-slate-700">
          <Bullet>① <span className="text-slate-900 font-medium">視覚的開示</span>: 投稿に「AI 生成」と分かるラベルを付与</Bullet>
          <Bullet>② <span className="text-slate-900 font-medium">機械可読メタデータ</span>: C2PA 等の標準形式</Bullet>
        </ul>
        <Note color="amber">
      ...
      {/* ── ベストプラクティス ─────────────── */}
      <Section icon={Sparkles} iconColor="#0891b2" title="運用ベストプラクティス（凍結回避＋伸び）">
        <ul className="space-y-2 text-[14px] text-slate-700">
          <Bullet>
            <span className="text-slate-900 font-medium">毎日同じ時間帯に投稿</span>。アルゴリズムは継続性を評価します
          </Bullet>
          <Bullet>
            <span className="text-slate-900 font-medium">投稿前に自分で軽く読み直す</span>。完全無人運用は X ban wave の主要ターゲット
          </Bullet>
          <Bullet>
            <span className="text-slate-900 font-medium">同じ文章を複数アカウントで使わない</span>。Xpresso が重複検知警告を出します
          </Bullet>
          <Bullet>
            <span className="text-slate-900 font-medium">AI 生成プロフィール画像を使わない</span>。実在感のあるアイコンを 1 つ用意
          </Bullet>
          <Bullet>
            <span className="text-slate-900 font-medium">通報されないこと</span> が最大の防御策。煽り投稿・規約スレスレの表現は避ける
          </Bullet>
        </ul>
      </Section>
      ...
      <div className="mt-6 flex items-center gap-3">
        <Link
          href="/guide"
          className="inline-flex items-center gap-1 text-[13px] text-slate-500 hover:text-slate-700 transition-colors"
        >
          <ArrowLeft size={12} /> 使い方ガイドに戻る
        </Link>
      </div>
...
function NGRow({ act, src, penalty, alt }: { act: string; src: string; penalty: string; alt?: boolean }) {
  return (
    <tr className={`border-t border-slate-900/10 ${alt ? 'bg-slate-900/[0.02]' : ''}`}>
      ...
      <td className="px-3 py-2.5 text-rose-600">{penalty}</td>
    </tr>
  );
}
...
function Note({ children, color = 'slate' }: { children: React.ReactNode; color?: 'slate' | 'amber' | 'rose' }) {
  const styles: Record<string, { bg: string; border: string; text: string }> = {
    slate: { bg: 'rgba(15,23,42,0.02)',  border: 'rgba(15,23,42,0.06)',  text: '#94a3b8' },
    amber: { bg: 'rgba(245,158,11,0.05)',   border: 'rgba(245,158,11,0.2)',    text: '#b45309' },
    rose:  { bg: 'rgba(239,68,68,0.04)',    border: 'rgba(239,68,68,0.15)',    text: '#dc2626' },
  };
