...
import { InstagramAccountCard } from '@/components/instagram-accounts/InstagramAccountCard';
import { InstagramAccountForm } from '@/components/instagram-accounts/InstagramAccountForm';
import { Plus, Loader2, BookOpen } from 'lucide-react';
import type { SocialAccount } from '@/lib/types';
import { apiFetch } from '@/lib/api-fetch';
import { PlatformIcon } from '@/components/ui/PlatformIcon';

export function InstagramAccountsClient() {
  ...
  const fetchAccount = async () => {
    try {
      const res = await apiFetch('/api/instagram-accounts');
  ...
  const handleRefresh = async (id: string) => {
    setRefreshing(true);
    try {
      const res = await apiFetch(`/api/instagram-accounts/${id}/refresh`, { method: 'POST' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await apiFetch(`/api/instagram-accounts/${id}`, { method: 'DELETE' });
    ...
    <div className="mb-8">
      <div className="mb-4 flex items-center justify-between gap-3 flex-wrap">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-[11px] text-pink-300 hover:underline shrink-0"
        >
          <BookOpen size={11} />
          登録マニュアル
        </Link>
      ...
      {loading ? (
        ...
        >
          ...
          <div className="text-center">
            <p className="text-[13px] font-medium text-slate-500">Instagramアカウントを登録</p>
            <p className="text-[11px] text-slate-700 mt-1">アクセストークンを登録して Instagram 投稿機能を有効化します</p>
          </div>
        </button>
      )}
