 ...
 *
 * 過去 24 時間で公開済みの投稿数を X / Threads / Instagram 別に返す。
 * BAN 回避ガイドのソフトリミット（X: 100/日、Threads: 250/24h、Instagram: 25/24h）の判定に使う。
 *
...
export async function GET(req: Request) {
  ...
  for (const row of (data ?? []) as any[]) {
    ...
    else if (platform === 'instagram') instagramCount += 1;
  ...
  return NextResponse.json({
    ...
    limits: { x: 100, threads: 250, instagram: 25 },
    /** 集計の時間窓 */
    windowHours: 24,
  });
}
