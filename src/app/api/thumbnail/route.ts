...
async function generateWithGeminiImage(
  ...
  if (logoList) {
    directives.push(`Logo (${logoList}): treat as the FOREGROUND / TOPMOST layer — composite ABOVE all items and background. Keep the original logo design crisp and undistorted, preserve transparency if present, do not crop, do not redraw, do not generate alternative text. Place as a moderate overlay (typically a corner) that remains clearly visible.`);
  }
