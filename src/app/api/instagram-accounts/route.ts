...
/** GET /api/instagram-accounts — ログインユーザーの Instagram アカウント一覧（マスク済み） */
...
/** POST /api/instagram-accounts — 新規登録（1ユーザー1件のみ） */
export async function POST(req: Request) {
  ...
  if (existing && existing.length > 0) {
    return NextResponse.json({ error: 'Instagramアカウントは1件のみ登録できます' }, { status: 409 });
  }

  // ── Instagram API で認証確立を確認 ──
  const verified = await verifyInstagramTokens({ access_token: body.access_token });
  if (!verified.ok) {
  ...
  const effectiveName     = body.name?.trim() || verified.user?.name || verified.user?.username || 'Instagramアカウント';
