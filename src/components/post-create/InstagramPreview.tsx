...
export function InstagramPreview({ caption, imagePreviews, instagramAccount }: InstagramPreviewProps) {
  ...
  if (!caption && imagePreviews.length === 0) {
    return (
      ...
      >
        <div
          className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-[#db2777]"
          style={{ background: 'rgba(219,39,119,0.1)', border: '1px solid rgba(219,39,119,0.22)' }}
        >
          <PlatformIcon platform="instagram" size={20} />
        </div>
        <p className="text-[16px] font-bold text-slate-700">Instagram プレビュー</p>
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span
            className="w-6 h-6 rounded-md flex items-center justify-center text-[#db2777]"
            style={{ background: 'rgba(219,39,119,0.14)', border: '1px solid rgba(219,39,119,0.3)' }}
          >
      ...
      <article className="px-4 pt-4 pb-3">
        ...
        {/* メディア（正方形） */}
        <div className="mt-3 rounded-xl overflow-hidden" style={{ border: '1px solid rgba(15,23,42,0.1)' }}>
          {imagePreviews.length === 0 ? (
            <div
              className="flex flex-col items-center justify-center gap-2 aspect-square text-slate-500"
              style={{ background: 'rgba(15,23,42,0.02)' }}
            >
              <ImageOff size={28} />
              <p className="text-[12px]">画像が必要です（Instagram は画像必須）</p>
            </div>
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
            <img
        ...
        {caption && (
          <p
            className="text-[15px] text-slate-900 whitespace-pre-wrap mt-2.5 leading-[1.6]"
        ...
        {/* メタ行 */}
        <p className="text-[13px] text-slate-500 mt-2">{caption.length} / 2,200 文字</p>
      </article>
