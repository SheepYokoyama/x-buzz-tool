  ...
  return (
    <div
      ...
      style={persona.is_active ? {
        ...
        boxShadow: '0 0 40px rgba(167,139,250,0.08), inset 0 1px 0 rgba(15,23,42,0.05), 0 16px 56px rgba(0,0,0,0.45)',
      } : undefined}
    >
      {/* Header */}
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[16px] font-semibold text-slate-800 leading-tight">{persona.name}</p>
            <p className="text-[13px] text-slate-500 mt-0.5">{persona.tone}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {persona.is_active && (
            <span
              className="text-[12px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1"
              style={{ background: 'rgba(52,211,153,0.1)', color: '#059669', border: '1px solid rgba(52,211,153,0.2)' }}
            >
              <span className="w-1.5 h-1.5 rounded-full bg-neon-green inline-block" style={{ boxShadow: '0 0 4px #059669' }} />
              使用中
            </span>
          )}
          {/* 編集ボタン */}
          ...
          {!persona.is_active && (
            <button
              ...
              className="w-7 h-7 rounded-lg flex items-center justify-center transition-all text-slate-600 hover:text-red-600"
              style={{ background: 'rgba(15,23,42,0.03)', border: '1px solid rgba(15,23,42,0.07)' }}
              title="削除"
            >
              <Trash2 size={12} />
      ...
      {/* Description */}
      <p className="text-[14px] text-slate-500 leading-relaxed">{persona.description}</p>

      {/* Style */}
      <div>
        <p className="section-label mb-2">スタイル</p>
        <p
          className="text-[13px] text-slate-600 rounded-xl px-3.5 py-2.5 leading-relaxed"
