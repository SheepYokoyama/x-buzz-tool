/**
 * Threads 画像投稿用の Supabase Storage ヘルパー
 *
 * Meta Threads API は画像の直接アップロードを受け付けず、公開URL（image_url）の
 * 指定のみをサポートする。そのため:
 *   1. 投稿APIに添付された画像を一旦 Supabase Storage の `threads-uploads` バケットに
 *      上げ、public URL を取得する。
 *   2. その URL を Meta Graph API に渡して投稿。
 *   3. 投稿成立後（成功/失敗にかかわらず）Storage 上のファイルは削除する。
 *
 * バケットは migration `20260521_threads_uploads_storage.sql` で作成済み。
 * 削除は best-effort（失敗してもログのみで握りつぶす）。
 */
import { randomUUID } from 'crypto';
import { getSupabaseAdmin } from '@/lib/supabase';

const BUCKET = 'threads-uploads';
...
export interface UploadedThreadsImage {
  /** バケット内の path（削除時に使用）*/
  path: string;
...
/**
 * 単一の画像ファイルを `threads-uploads/{userId}/{uuid}.{ext}` へアップロードし、
 * Meta Graph API に渡すための public URL を返す。
 */
export async function uploadThreadsImage(params: {
  userId: string;
  file: File;
}): Promise<UploadedThreadsImage> {
 ...
 */
export async function deleteThreadsImages(paths: string[]): Promise<void> {
  if (paths.length === 0) return;
  try {
    const { error } = await getSupabaseAdmin().storage.from(BUCKET).remove(paths);
    if (error) {
      console.error('[threads-storage] 削除失敗:', error.message, paths);
    }
  } catch (err) {
    console.error('[threads-storage] 削除中に例外:', err);
  }
}
